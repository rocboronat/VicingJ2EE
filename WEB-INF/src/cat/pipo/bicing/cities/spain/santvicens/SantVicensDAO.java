	...
	public static boolean loadStations() throws Exception {
		...
		try {
			...
			if (html != null && !html.equals("")) {
				no_tags = html.replaceAll(HTML_TAG, ""); // Elimina etiquetas
															// HTML

				Short id = 0;
				// Obtiene la dirección (puede haber direcciones del estilo:
				// PLACE - SUBPLACE
				String direccion = no_tags.substring(
						no_tags.indexOf(DIR) + DIR.length(),
						no_tags.indexOf(DIREND));
				// direccion =
				// direccion.substring(0,direccion.lastIndexOf("-")-1);
				// Obtiene un String con formato n/m - n=bicis, m=huecos
				String infoStation = no_tags.substring(
						no_tags.indexOf(BIKESINFO) + BIKESINFO.length(),
						no_tags.indexOf(BIKESINFOEND));

				// Elimina del string principal lo ya parseado
				no_tags = no_tags.substring(no_tags.indexOf(BIKESINFOEND) + 1,
						no_tags.length());

				while (no_tags.indexOf(DIR) > -1
						&& no_tags.indexOf(BIKESINFO) > -1) {
					// Pasa las strings a bytes
					Byte bikes, holes;
					if (infoStation != "") {
						bikes = Byte.parseByte(infoStation.substring(0,
								infoStation.indexOf("/")));
						holes = Byte.parseByte(infoStation.substring(
								infoStation.indexOf("/") + 1,
								infoStation.length()));
					} else {
						bikes = holes = 0;
					}

					// Crea una nueva estación (las coordenadas no las
					// proporciona la web)
					Station s = new Station(id++, direccion, bikes, holes, 0.0,
							0.0);
					stations.add(s);

					// Obtiene los datos de la siguiente estación
					direccion = no_tags.substring(
							no_tags.indexOf(DIR) + DIR.length(),
							no_tags.indexOf(DIREND));
					// direccion =
					// direccion.substring(0,direccion.lastIndexOf("-")-1);
					infoStation = no_tags
							.substring(
									no_tags.indexOf(BIKESINFO)
											+ BIKESINFO.length(),
									no_tags.indexOf(BIKESINFOEND));
					no_tags = no_tags
							.substring(no_tags.indexOf(BIKESINFOEND) + 1,
									no_tags.length());
				}
				Byte bikes = Byte.parseByte(infoStation.substring(0,
						infoStation.indexOf("/")));
				Byte holes = Byte.parseByte(infoStation.substring(
						infoStation.indexOf("/") + 1, infoStation.length()));
	...
	private static String conexionGET(String u) {
		...
		try {
			...
			rd = new BufferedReader(new InputStreamReader(
					conn.getInputStream(), Charset.forName("latin1")));
